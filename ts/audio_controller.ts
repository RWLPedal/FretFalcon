...
export class AudioController {
  ...
  playStrumDown(accent: boolean): void {
    try {
      ...
      tflt.type = 'lowpass'; tflt.frequency.value = 220;
      const tgain = ctx.createGain();
      tgain.gain.setValueAtTime(vol * 0.4, now);
      tgain.gain.exponentialRampToValueAtTime(0.001, now + tDur);
      tsrc.connect(tflt); tflt.connect(tgain); tgain.connect(ctx.destination);
      tsrc.start(now); tsrc.stop(now + tDur + 0.01);
    } catch (e) { console.warn('AudioController: strum down error', e); }
  }
  ...
  playStrumUp(accent: boolean): void {
    try {
      const ctx = volumeManager.getAudioContext();
      const now = ctx.currentTime;
      const vol = volumeManager.getVolume() * (accent ? 0.85 : 0.60);
      ...
      src.connect(flt); flt.connect(gain); gain.connect(ctx.destination);
      src.start(now); src.stop(now + dur + 0.01);
    } catch (e) { console.warn('AudioController: strum up error', e); }
  }
  ...
  playStrumChuck(): void {
    ...
    } catch (e) { console.warn('AudioController: strum chuck error', e); }
  }
}
